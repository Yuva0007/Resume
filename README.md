# YUVARAJ M

**Software Intern**

 8122775039  
 [Mail](yuvaraj.m.7891@gmail.com) | [LinkedIn](https://www.linkedin.com/in/yuvaraj-m-bb42ab354/) | [GitHub](https://github.com/Yuva0007)

---

## Profile

A highly motivated Computer Science undergraduate with a strong foundation in programming, data structures, and algorithms. Skilled in **C++**, **Java**, and problem-solving. Passionate about building efficient software solutions and eager to apply academic knowledge to real-world challenges. Seeking opportunities to learn, grow, and contribute in a dynamic tech environment.

---

## Education

### Pem School of Excellence  
**Higher Secondary**  
**2017 – 2023**  
**GPA :** 70 / 100

### Sri Krishna College of Engineering and Technology  
**B.E. Computer Science and Engineering**  
**2023 – 2027**  
**GPA :** 7.5 / 10

---

## Projects

### Smart Tampering Detection System (IoT Project)
- Developed an IoT-based tamper detection system to monitor physical disturbances in real time using sensors.
- Implemented instant alert mechanisms to enhance security and system reliability.

### Alumni Management System
- Built a full-stack alumni networking platform using Spring Boot and React.
- Features include messaging, event posting, and profile management.
- Implemented secure authentication, role-based access, and scalable REST APIs.

### Sales Dashboard
- Created an interactive Excel-based sales dashboard using pivot tables, charts, and slicers.
- Automated data analysis to track revenue, top-selling products, and monthly performance trends.

---

## Skills

### Programming
- Java  
- C++  
- SQL  

### Tools & Technologies
- Spring Boot  
- React  
- Git  
- GitHub  
- Excel  

### Core Concepts
- Data Structures and Algorithms  
- Operating Systems  
- Computer Networks  

---

## Hobbies

- Listening to Music  
- Badminton & Cricket  
- Gym & Swimming  
- Coding  

---

## Certifications

- Udemy Certified Java Developer  
- Udemy Certified C++ Developer  
- Udemy Certified Excel Analyst  
- Google Cloud Computing Certificate  
- AWS Cloud Certificate  

---

## Languages

- Tamil (Fluent)  
- English (Fluent)  
- Hindi (Basic / Elementary)


